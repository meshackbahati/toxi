...
Server-side rendering with the Toxi template engine.

## Installation

```toml
[dependencies]
toxi = { version = "1.0", features = ["templates"] }
```
...
async fn home() -> Result<ToxiResponse> {
...
async fn blog_index(State(db): State<Database>) -> Result<ToxiResponse> {
...
Complete documentation at [docs.rs/toxi](https://docs.rs/toxi)
