...
```mermaid
graph LR
    CLI[toxi-cli] --> Core[toxi-core]
    CLI --> Router[toxi-router]
    CLI --> DB[toxi-db]
    CLI --> Migrate[toxi-migrate]
    
    Router --> Core
    Middleware[toxi-middleware] --> Core
    Auth[toxi-auth] --> Core
    Auth --> DB
    DB --> Core
    Queue[toxi-queue] --> Core
    Cache[toxi-cache] --> Core
    Realtime[toxi-realtime] --> Core
...
- `ToxiRequest`: Alias for `Request<Incoming>`.
- `ToxiResponse`: Alias for `Response<BoxBody>`.
...
[Load Balancer]
      |
   [Toxi Server 1] [Toxi Server 2] [Toxi Server N]
      |                     |                     |
      +---------------------+---------------------+
                            |
                    [Shared Database]
