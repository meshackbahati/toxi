...
Configuration and environment variable management for Toxi applications. Loads `toxi.toml`, `.env` files, and OS environment variables with typed config sections and namespaced table support.
...
| Method | Description |
|--------|-------------|
| `Config::load()` | Load from `toxi.toml`, fallback to default |
