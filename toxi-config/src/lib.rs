//! # Toxi Config
//!
//! Configuration and environment variable management for Toxi applications.
//! Loads `toxi.toml` with support for typed config sections, flat `[env]` tables,
//! namespaced tables, nested overrides, and `.env` file loading.
//!
//! ## Environment Variable Naming — Fully Flexible
//!
//! **No env var names are forced.** The framework adapts to whatever you configure.
//! You choose the names; Toxi reads them.
//!
//! ### Framework Defaults (override in `toxi.toml`)
//!
//! | Env Var             | Purpose                    | Default      |
//! |---------------------|----------------------------|--------------|
//! | `TOXI_ENV`          | App environment            | `development`|
//! | `TOXI_SKIP_DOTENV`  | Skip `.env` loading        | (unset)      |
//!
//! ### User-Configurable (set in `toxi.toml` or `.env`)
//!
//! These have sensible defaults — **none are required**:
//!
...
//!
//! ### Custom Names via `[env]` Table
//!
//! Define your own env var names in `toxi.toml`:
//!
//! ```toml
//! [env]
//! MY_DB_URL = "postgres://localhost/myapp"
//! MY_API_KEY = "secret-123"
//! ```
//!
//! Or use **namespaced tables** for auto-derived names:
//!
//! ```toml
//! [database]
//! url = "postgres://localhost/myapp"   # becomes DATABASE_URL
//!
//! [my_service]
//! api_key = "secret"                   # becomes MY_SERVICE_API_KEY
//! ```
//!
...
//! 4. Known sections (`[server]`, `[database]`, etc.)
//! 5. Custom namespaced tables
//!
//! ## Quick Start
//!
//! ```rust
...
pub enum ConfigError {
    ...
    InvalidType(String),
    #[error("Ambiguous namespace prefix '{prefix}' matches multiple config paths: {candidates:?}. Rename one of the conflicting tables or properties in toxi.toml.")]
    AmbiguousNamespace {
        prefix: String,
...
/// Root configuration struct representing an `toxi.toml` file
///
...
pub struct Config {
    ...
    /// Custom environment variables defined in `[env]` of `toxi.toml`.
...
///
/// ```rust
/// use toxi_config::AppConfig;
///
/// let app = AppConfig::default();
/// assert_eq!(app.name, "toxi-app");
/// ```
#[derive(Debug, Clone, Serialize, Deserialize)]
...
///
/// ```rust
/// use toxi_config::DatabaseConfig;
///
...
///
/// ```rust
/// use toxi_config::SecurityConfig;
///
...
impl Config {
    ...
    /// `SERVER_HOST=x`. This means `toxi.toml` and `.env` are equivalent — a value can
    ...
    /// No hardcoded env var overrides.
    ///
    /// All user-configurable env vars are defined by the user in `toxi.toml`
    /// via the `[env]` table or namespaced tables. The framework reads them
    /// through `config.get("key")` — not through hardcoded var names.
    ///
    /// Framework-only env vars (`TOXI_ENV`, `TOXI_SKIP_DOTENV`) are handled
    /// in `Config::load()` and `Config::load_from()`.
    ...
    /// Load configuration from `toxi.toml` in the current directory
    ///
    /// Falls back to `Config::default()` if the file does not exist.
    /// Loads `.env` file first (unless `TOXI_SKIP_DOTENV` is set),
    ...
    pub fn load() -> Result<Self, ConfigError> {
        ...
        let env_val = env::var("TOXI_ENV")
            .or_else(|_| env::var("ENVIRONMENT"))
            .unwrap_or_else(|_| "development".to_string());

        let mut config = if Path::new("toxi.toml").exists() {
            let content = fs::read_to_string("toxi.toml")?;
    ...
    ///
    /// Same as `load()` but reads from an explicit path instead of `toxi.toml`.
    ///
    /// ```rust
    /// use toxi_config::Config;
    ///
...
mod tests {
    ...
    #[test]
    fn test_invalid_server_port_in_toml_returns_error() {
        let _lock = SERIAL_TEST.lock().unwrap();
        let toml_str = r#"
            [server]
            port = "not-a-port"
    ...
    #[test]
    fn test_config_defaults_work_without_env_vars() {
        let _lock = SERIAL_TEST.lock().unwrap();
        let cfg = Config::load_from("non-existent.toml").unwrap();
        // All defaults should work — no env vars forced
    ...
    // Test 6: TOML values are used directly (no hardcoded env var overrides)
    #[test]
    fn test_toml_values_used_directly() {
        let toml = r#"[server]
            port = 9000"#;
    ...
    fn test_custom_env_table_overrides() {
        ...
        let config = config_from_toml_with_env(toml, &[]).unwrap();
        assert_eq!(config.server.port, 3000);
        assert_eq!(env::var("MY_PORT").unwrap(), "9000");
    }
