...
Lightweight ORM and migration helpers for Toxi, built on `sqlx::Any` with concrete pool escape hatches.
...
- `#[derive(Model)]` CRUD generation via `toxi-macros`.
...
```rust
use toxi_db::{DbPool, Model, Pagination, SortDirection, sqlx};
...
**v3.1.0+**: `DbPool` now stores concrete connection pools alongside the `AnyPool` abstraction, enabling use of PostgreSQL/MySQL/SQLite-specific features that the `Any` driver doesn't support.
