...
use toxi_core::{Router, Result};
use juniper::RootNode;
use http_body_util::BodyExt;

/// GraphQL handler for Toxi
...
impl GraphQLHandler {
    ...
    pub fn mount_at(&self, router: &mut Router, endpoint: &'static str) -> Result<()> {
        ...
        router.post(endpoint, move |req: toxi_core::ToxiRequest| {
            ...
            async move {
                ...
                let body_bytes = req.into_body()
                    .collect()
                    .await
                    .map_err(|e| toxi_core::Error::BadRequest(format!("Failed to read body: {}", e)))?
                    .to_bytes();
                ...
                let graphql_request: juniper::http::GraphQLBatchRequest = serde_json::from_slice(&body_bytes)
                    .map_err(|e| toxi_core::Error::BadRequest(format!("Invalid GraphQL request: {}", e)))?;
        ...
        router.get(endpoint, move |_req: toxi_core::ToxiRequest| {
            let endpoint = endpoint;
            async move {
                let html = juniper::http::playground::playground_source(endpoint, None);
                Ok(toxi_core::ToxiResponse::html(html))
            }
        });
