...
// Embed the Toxi SVG logo at compile time so `toxi new` can write it
// to the generated project's `public/images/` directory.
const TOXI_LOGO_SVG: &str = include_str!("../templates/toxi.svg");
...
pub fn create_project(
    ...
    println!(
        "{}",
        format!("Initializing new Toxi project: {}", name)
            .green()
            .bold()
    );
...
fn create_test_file(path: &Path, p_type: ProjectType) -> std::io::Result<()> {
    let test_content = match p_type {
        ProjectType::Serverless => r#"use toxi_testing::{TestRequest, TestResponse, test_router};
use toxi::prelude::*;
...
async fn test_health_endpoint() {
    ...
    let req = TestRequest::get("/").build_toxi();
    let resp = server.call(req).await.unwrap();

    let test_resp = TestResponse::from_toxi_response(resp).await;
    assert!(test_resp.is_success());
}
"#,
        ProjectType::Microservice => r#"use toxi_testing::{TestRequest, TestResponse, test_router};
use toxi::prelude::*;
...
async fn test_health_check() {
    ...
    let mut server = test_router(router);
    let req = TestRequest::get("/health").build_toxi();
    let resp = server.call(req).await.unwrap();
    let test_resp = TestResponse::from_toxi_response(resp).await;
    assert!(test_resp.is_success());
}
...
async fn test_status_endpoint() {
    ...
    let mut server = test_router(router);
    let req = TestRequest::get("/api/v1/status").build_toxi();
    let resp = server.call(req).await.unwrap();
    let test_resp = TestResponse::from_toxi_response(resp).await;
    assert!(test_resp.is_success());
}
"#,
        _ => r#"use toxi_testing::{TestRequest, TestResponse, test_router};
use toxi::prelude::*;
...
[dependencies]
# Toxi — only the features this project type needs: {features_comment}.
# To add more: toxi = {{ features = ["auth", "realtime", ...] }}
        toxi = {{ version = "{TOXI_VERSION}", {features_list} }}
...
[dev-dependencies]
toxi-testing = "{TOXI_VERSION}"
"#,
        name = name,
        TOXI_VERSION = TOXI_VERSION,
...
├── toxi.toml      # Framework + provider config
└── .env.example"#,
...
├── toxi.toml       # Framework configuration
...
└── tests/             # Integration tests"#,
    ...
    let deploy_targets = match p_type {
        ProjectType::Serverless => r#"| `toxi deploy --target aws-lambda` | AWS Lambda + API Gateway |
| `toxi deploy --target cloudflare` | Cloudflare Workers |
| `toxi deploy --target docker`     | Container (Lambda, Cloud Run, ECS) |
...
Generated by `toxi new` as an Toxi **{project_kind}**.
...
| Command              | Description                          |
|----------------------|--------------------------------------|
| `toxi dev`        | Start dev server with hot-reload     |
| `toxi doctor`     | System health check                  |
| `toxi deploy`     | Generate deployment artifacts        |
| {migrate_cmd}
| `toxi make model` | Scaffold a new model                 |
"#,
        name = name,
        project_kind = project_kind,
        quick_start_cmds = match p_type {
            ProjectType::Serverless => "toxi dev",
            _ => "toxi migrate\ntoxi dev",
        },
        structure = structure,
        deploy_targets = deploy_targets,
        migrate_cmd = match p_type {
            ProjectType::Serverless => "`toxi run script.rs` | Run a standalone Rust script      |",
            _ => "`toxi migrate`    | Run pending migrations               |",
        },
    );
...
docker run -e TOXI_SERVERLESS=1 -p 8080:8080 PROJECT_NAME:latest
```
...
4. Set environment variables in the task definition: `SERVER_HOST=0.0.0.0`, `TOXI_ENV=production`.
...
[Unit]
Description=PROJECT_NAME Toxi server
...
This guide covers deployment options for the {name} Toxi project.
...
| `TOXI_ENV` | Environment name | `development` |
...
set `TOXI_LOG_FORMAT=json`.

## Production Checklist

- [ ] Set `TOXI_ENV=production`
...
#
# .env files also work and take priority over toxi.toml values.
...
"#,
    ...
    fs::write(path.join("toxi.toml"), content)
}
...
// Use `toxi make <kind> <name>` to scaffold new modules.
mod routes;
mod controllers;
mod middleware;
mod models;
...
async fn main() -> Result<()> {
    // ── 1. Config ──────────────────────────────────────────────────────
    // Load configuration from toxi.toml, .env, and OS env overrides.
...
"#
        ...
        ProjectType::Serverless => {
            r#"// Toxi Serverless Function — entry point.
//
// Handlers live in src/handlers/ — each file can be independently deployed.
// main() is for local development only; production uses TOXI_SERVERLESS=1
...
async fn main() -> Result<()> {
    ...
    if std::env::var("TOXI_SERVERLESS").is_ok() {
    ...
    // ── 2. Router ──────────────────────────────────────────────────────
    let config = toxi::config::Config::default();
    let mut app = Application::new(config);
    ...
    let module_files: &[(&str, &str)] = &[
        ("src/models/mod.rs",
         "// Data models and ORM structs are defined here.\n\
          // Use `toxi make model User name:string email:string` to scaffold.\n"),
        ("src/controllers/mod.rs",
         "// Controllers handle HTTP requests and return responses.\n\
          // Use `toxi make controller Auth` to scaffold.\n"),
        ("src/services/mod.rs",
         "// Services contain business logic, keeping controllers thin.\n\
          // Use `toxi make service Payment` to scaffold.\n"),
        ("src/middleware/mod.rs",
         "// Middleware runs before/after request handlers.\n\
          // Use `toxi make middleware RequireAuth` to scaffold.\n"),
        ("src/validators/mod.rs",
         "// Validators check and sanitize user input.\n\
          // Use `toxi make validator CreateUser` to scaffold.\n"),
        ("src/jobs/mod.rs",
         "// Background jobs for async processing (emails, reports, etc.).\n\
          // Use `toxi make job SendWelcomeEmail` to scaffold.\n"),
        ("src/policies/mod.rs",
         "// Authorization policies control access to resources.\n\
          // Use `toxi make policy PostPolicy` to scaffold.\n"),
        ("src/events/mod.rs",
         "// Event handlers react to domain events.\n\
          // Use `toxi make event UserRegistered` to scaffold.\n"),
    ];
...
// is where `toxi make route` inserts new route bindings automatically.
//
// Template rendering:
...
use toxi::prelude::*;
use toxi::template::{Context, TemplateContext};
use std::sync::Arc;
...
"#
    ...
    if let ProjectType::Fullstack = p_type {
        ...
        let css_content = r#"/* Toxi default styles.
 *
...
<head>
  ...
  <title>Hello, Toxi</title>

  <!-- Favicon — the Toxi logo from public/images/.
       Delete this line or point it to your own icon. -->
  <link rel="icon" type="image/svg+xml" href="/images/toxi.svg">
...
<body>
  <div class="container">
    <!-- Logo — served from public/images/toxi.svg.
         Remove this <img> or replace the src with your own logo. -->
    <img class="logo" src="/images/toxi.svg" alt="Toxi logo">

    <h1>Hello, {{ name }}!</h1>
    <p>Your Toxi app is running. Start building something great.</p>
  </div>
...
// Use `toxi make model <name> <field>:<type>` to scaffold.
"#,
    )?;
...
// Import and use shared config::Config from toxi.
pub use toxi::config::Config;
"#,
    )?;
...
"#,
    ...
    fs::write(
        path.join("src/config/mod.rs"),
        r#"use toxi::config::Config;
use std::env;

/// Load configuration with environment-aware overrides.
/// Tries `config/{ENV}.toml` first, falls back to `toxi.toml`.
pub fn load() -> Config {
    let env_name = env::var("TOXI_ENV").unwrap_or_else(|_| "dev".into());
...
"#,
    ...
    // ── infra/aws/main.tf ─────────────────────────────────────────────
    let tf_content = r#"# Terraform for PROJECT_NAME — Toxi serverless functions.
