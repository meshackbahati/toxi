...
FUNCTION_NAME="${1:-my-toxi-function}"
...
"#;
    ...
    let tf_content = r#"# Terraform module for Toxi Lambda function
# Update the image_uri after your first deploy.
resource "aws_lambda_function" "toxi_fn" {
  function_name = "toxi-serverless"
  package_type  = "Image"
  image_uri     = "${var.aws_account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/toxi-serverless:latest"
...
fn generate_cloudflare(out: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let wrangler = r#"name = "toxi-function"
main = "src/main.rs"
