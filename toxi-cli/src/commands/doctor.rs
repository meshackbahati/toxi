...
/// Find project root by searching for Cargo.toml or toxi.toml in parent directories
fn find_project_root() -> Option<std::path::PathBuf> {
    let mut current = std::env::current_dir().ok()?;
    loop {
        if current.join("Cargo.toml").exists() || current.join("toxi.toml").exists() {
            return Some(current);
        }
        if !current.pop() {
            return None;
...
pub fn run_doctor() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Check if in an Toxi project (search parent directories)
    ...
    // Check toxi.toml or config (search parent directories)
    print!("Checking configuration... ");
    if let Some(root) = find_project_root() {
        if root.join("toxi.toml").exists() {
            println!("✅ toxi.toml found");
    ...
    // Check framework env vars
    println!("\nFramework environment variables (all optional):");
    check_env_var("TOXI_ENV", "Application environment (development/testing/production)");
    check_env_var("TOXI_DEBUG", "Enable debug output");
    check_env_var("TOXI_SKIP_DOTENV", "Skip .env file loading");

    // Check user-configurable env vars (naming is flexible via toxi.toml)
    println!("\nUser-configurable variables (set in toxi.toml or .env):");
    check_env_var("DATABASE_URL", "Database connection string");
    check_env_var("REDIS_URL", "Redis connection string");
    check_env_var("JWT_SECRET", "JWT signing secret");
    println!("  (env var names are flexible — configure custom names in [env] of toxi.toml)");
...
fn check_env_var(name: &str, description: &str) {
    ...
    match env::var(name) {
        Ok(value) => {
            ...
            println!("✅ {} ({})", masked, description);
        }
        Err(_) => println!("⚠️  Not set ({})", description),
    }
}
