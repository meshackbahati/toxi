...
/// 2. **Project mode**: Run a Rust file inside an existing Toxi/Cargo
...
/// toxi run src/bin/migrate_users.rs
///
...
fn run_standalone(
    ...
    let mut deps: Vec<(String, String)> = vec![
        ("toxi".into(), format!("\"{}\"", env!("CARGO_PKG_VERSION"))),
        ("tokio".into(), "{ version = \"1\", features = [\"full\"] }".into()),
        // Direct sub-crate deps so `use toxi_auth::*` style imports work.
        ("toxi-core".into(),       "\"*\"".into()),
        ("toxi-middleware".into(), "\"*\"".into()),
        ("toxi-config".into(),     "\"*\"".into()),
        ("toxi-db".into(),         "\"*\"".into()),
        ("toxi-auth".into(),       "\"*\"".into()),
        ("toxi-queue".into(),      "\"*\"".into()),
        ("toxi-cache".into(),      "\"*\"".into()),
        ("toxi-realtime".into(),   "\"*\"".into()),
        ("toxi-template".into(),   "\"*\"".into()),
        ("toxi-mail".into(),       "\"*\"".into()),
        ("toxi-storage".into(),    "\"*\"".into()),
        ("toxi-security".into(),   "\"*\"".into()),
        ("toxi-macros".into(),     "\"*\"".into()),
        ("toxi-utils".into(),      "\"*\"".into()),
        ("toxi-openapi".into(),    "\"*\"".into()),
        ("toxi-graphql".into(),    "\"*\"".into()),
        ("toxi-plugin".into(),     "\"*\"".into()),
...
fn dirs_cache() -> PathBuf {
    if let Ok(xdg) = std::env::var("XDG_CACHE_HOME") {
        PathBuf::from(xdg).join("toxi-run")
    } else if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(".cache").join("toxi-run")
    } else {
        std::env::temp_dir().join("toxi-run-cache")
    }
}
