...
#[command(name = "toxi")]
#[command(version)]
#[command(about = "Toxi Framework CLI", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Serve {
        ...
        #[arg(long)]
        port: Option<u16>,
        /// Environment override passed to the project as TOXI_ENV
        #[arg(long)]
        env: Option<String>,
...
async fn main() {
    ...
    let result: Result<(), Box<dyn std::error::Error>> = match cli.command {
        ...
        Commands::Version => {
            println!("toxi {}", env!("CARGO_PKG_VERSION"));
            Ok(())
        }
        Commands::Tinker => {
