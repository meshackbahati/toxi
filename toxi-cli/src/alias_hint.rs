...
fn get_hint_file() -> PathBuf {
    ...
    cargo_home.join(".toxi_alias_hint_shown")
}
...
pub fn print_alias_hint() {
    ...
    if shell.contains("zsh") {
        eprintln!("  echo \"alias oxi='toxi'\" >> ~/.zshrc");
        eprintln!("  source ~/.zshrc");
    } else if shell.contains("bash") {
        eprintln!("  echo \"alias oxi='toxi'\" >> ~/.bashrc");
        eprintln!("  source ~/.bashrc");
    } else {
        eprintln!("  # Add to your shell config:");
        eprintln!("  alias oxi='toxi'");
    }
    
    if cfg!(windows) {
        eprintln!("\nWindows (PowerShell):");
        eprintln!("  Set-Alias oxi toxi  # Add to $PROFILE");
        eprintln!("\nWindows (CMD):");
        eprintln!("  doskey oxi=toxi");
    }
