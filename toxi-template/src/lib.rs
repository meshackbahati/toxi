//! # Toxi Template
//!
//! Server-side rendering engine for Toxi applications.
//!
//! Provides a template parser, renderer with filters, and static file
//! serving. Templates use a syntax similar to Jinja2/Tera with
//! `{% %}` for logic and `{{ }}` for expressions.
//!
//! ## Quick Start
//!
//! ```rust,ignore
//! use toxi_template::{Renderer, StaticFiles};
//!
//! let renderer = Renderer::new("./templates");
//! let html = renderer.render("index.html", &context)?;
//!
//! // Serve static files
...
/// Template renderer (evaluates AST with context data).
pub mod renderer;

/// Built-in template filters (`upper`, `lower`, `date`, `json`, etc.).
pub mod filters;
...
impl TemplateEngine {
    ...
    pub fn render_response(&self, name: &str, context: &Context) -> Result<ToxiResponse> {
        let html = self.render(name, context)?;
        Ok(ToxiResponse::html(html))
    }
...
impl Template {
    ...
    pub fn render_response(&self, context: &Context) -> Result<ToxiResponse> {
