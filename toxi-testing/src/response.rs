...
impl TestResponse {
    ...
    /// Convert from an Toxi response for test assertions.
    pub async fn from_toxi_response(response: toxi_core::ToxiResponse) -> Self {
...
mod tests {
    ...
    #[tokio::test]
    async fn from_toxi_response_collects_body() {
        let response = toxi_core::ToxiResponse::text("hello");
        let test_response = TestResponse::from_toxi_response(response).await;
