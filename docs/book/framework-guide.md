# Framework Guide: Building Real Applications with Toxi (v3.1.0)

This guide is a practical map for building production services with Toxi.

## How Toxi is structured

At a high level:

1. `toxi-core` handles HTTP primitives (request/response/router/server).
2. Feature crates layer capabilities (db/auth/queue/realtime/cache/storage/etc).
3. `toxi` umbrella crate re-exports these capabilities behind feature flags.
...
Toxi uses `toxi.toml` as its primary config file, with optional `.env` support for secrets and per-developer overrides.
...
2. `.env` file values
3. `toxi.toml [env]` table
4. `toxi.toml` section defaults
5. Hardcoded defaults

Blank `.env` entries (`KEY=`) are treated as unset, so the `[env]` fallback applies. Set `TOXI_SKIP_DOTENV=1` to skip `.env` loading entirely.
...
- hash passwords with `toxi-auth` helpers
- validate and sanitize untrusted input (`toxi-security`)
...
- handler tests with `toxi-testing` test server/request/response
