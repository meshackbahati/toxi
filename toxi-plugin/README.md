...
Plugin system for the Toxi web framework.
...
`toxi-plugin` provides a flexible plugin system that enables extending Toxi applications with additional functionality. The plugin architecture supports dynamic loading, lifecycle management, and hook-based extension points. Plugins can add routes, middleware, database migrations, and more to your Toxi application.
...
```rust
use toxi_plugin::{Plugin, PluginInfo, HookRegistry, PluginResult};
use toxi::prelude::*;
...
```rust
use toxi_plugin::{HookRegistry, PluginContext};
...
```rust
use toxi_plugin::{PluginManager, PluginLoader};
...
```rust
use toxi_plugin::{Plugin, PluginDependencies};
