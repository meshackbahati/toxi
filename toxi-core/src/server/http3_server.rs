...
where
    S: Service<ToxiRequest, Response = ToxiResponse, Error = crate::error::Error> 
        + Clone 
        + Send 
        + Sync 
        + 'static,
    S::Future: Send + 'static,
{
    ...
    async fn handle_request(
        ...
        // Convert H3 request to ToxiRequest
        ...
        let toxi_req = Request::from_parts(parts, body);

        // Process with the service
        let response = service.call(toxi_req).await
            .map_err(|e| crate::error::Error::InternalServerError(e.to_string()))?;

        // Convert ToxiResponse to H3 response
