...
pub mod email_verification {
    use rand::Rng;
    use toxi_db::sqlx::Row; // ✅ REQUIRED for try_get()
    ...
    pub async fn create_token<D: toxi_db::Database + ?Sized>(
        db: &D,
        user_id: i64,
    ) -> toxi_db::Result<String> {
    ...
    pub async fn verify_email<D: toxi_db::Database + ?Sized>(
        db: &D,
        token: &str,
    ) -> toxi_db::Result<bool> {
        let query = toxi_db::sqlx::query(
    ...
    pub async fn is_verified<D: toxi_db::Database + ?Sized>(
        db: &D,
        user_id: i64,
    ) -> toxi_db::Result<bool> {
        let query = toxi_db::sqlx::query(
...
pub mod password_reset {
    ...
    pub async fn verify_token<D: toxi_db::Database + ?Sized>(
        db: &D,
        token: &str,
    ) -> toxi_db::Result<Option<i64>> {
        let now = chrono::Utc::now().timestamp();
    ...
    pub async fn consume_token<D: toxi_db::Database + ?Sized>(
        db: &D,
        token: &str,
    ) -> toxi_db::Result<()> {
        let query = toxi_db::sqlx::query(
    ...
    pub async fn cleanup_expired<D: toxi_db::Database + ?Sized>(
        db: &D,
    ) -> toxi_db::Result<()> {
        let now = chrono::Utc::now().timestamp();
        let query = toxi_db::sqlx::query(
...
pub mod two_factor {
    ...
    /// Enable 2FA for user
    pub async fn enable<D: toxi_db::Database + ?Sized>(
        db: &D,
        user_id: i64,
        secret: &str,
    ) -> toxi_db::Result<()> {
    ...
    pub async fn disable<D: toxi_db::Database + ?Sized>(
        db: &D,
        user_id: i64,
    ) -> toxi_db::Result<()> {
        let query = toxi_db::sqlx::query(
    ...
    pub async fn get_secret<D: toxi_db::Database + ?Sized>(
        db: &D,
        user_id: i64,
    ) -> toxi_db::Result<Option<String>> {
        let query = toxi_db::sqlx::query(
