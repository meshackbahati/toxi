use toxi_auth::{Role, Permission, AuthorizationService};
use toxi_db::{Model, Database, DatabaseType, DbTransaction, Result};
...
impl Database for MockDb {
    ...
    fn inspector(&self) -> Box<dyn toxi_db::DbInspector> {
        struct MockInspector;
        #[async_trait]
        impl toxi_db::DbInspector for MockInspector {
            async fn get_table_schema(&self, _table_name: &str) -> Result<Option<toxi_db::TableSchema>> { Ok(None) }
