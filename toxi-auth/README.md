...
Authentication and authorization for Toxi (RBAC, JWT, OAuth2, 2FA, API keys).
...
`toxi-auth` provides a comprehensive authentication and authorization system for the Toxi web framework. It includes JWT token management, secure password hashing, role-based access control, API key authentication, and two-factor authentication.
...
```rust
use toxi_auth::security::hasher::Hasher;
...
```rust
use toxi_auth::authorization::{Role, Permission};
...
```rust
use toxi_auth::api_key::{ApiKey, ApiKeyManager};
...
```rust
use toxi_auth::security::totp::{generate_secret, verify_code};
...
```rust
use toxi_auth::oauth2::{GoogleProvider, OAuth2Config};
...
```rust
use toxi_core::{Request, Response, Middleware};
use toxi_auth::middleware::AuthMiddleware;

// Use authentication middleware to protect routes
// This would typically be integrated with Toxi's middleware system
```
